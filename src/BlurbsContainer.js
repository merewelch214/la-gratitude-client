...
class BlurbsContainer extends React.Component {
    ...
    render() {
        ...
        console.log(current_entries[0] === this.state.journal_entries[0])

        return (
            <div>
                {current_entries.map(journal_entry => < Blurb {...journal_entry} key={journal_entry.id} /> )}<br />
                <button 
                    disabled={current_entries[0] === this.state.journal_entries[0]} 
                    onClick={this.previous}> Previous </button>
                <button 
                    disabled={current_entries[current_entries.length - 1] === this.state.journal_entries[this.state.journal_entries.length - 1]} 
